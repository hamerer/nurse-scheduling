...
echo "<tr>";
$sql = "SELECT  nurse,color,enable  FROM nurse ";
...
while ($rows=mysql_fetch_array($result)){
	...
	if ($rows[2]) { 
		...
		echo  "<div id='$id'><label><input type=checkbox  name=device[]  value=$rows[0]><span>$rows[0]</span></label></div>";
  	} else {
		echo  "<div id='ck-button'><label><input type=hidden   name='$device'  value=$rows[0]><span>$rows[0]</span></label></div>";
	}
	echo "</td>";
}
echo "</tr>";
