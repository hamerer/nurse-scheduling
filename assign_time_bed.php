...
$parm_error_log = '/tmp/assign_time_bed.log';
...
while ($rows=mysql_fetch_array($result)) {
	...
	if (!$rows[2]) {	
	$update_sql = "UPDATE bed set day_nurse='$nurse',day='1'  WHERE   name='$rows[0]' AND bed='$rows[1]' "; 
...
if(isset($_POST[ints])){
	...
	if (!$srow[2]) {
	$update_sql = "UPDATE bed set day_nurse='$nurse' ,day='1'  WHERE   bed='$srow[1]'  "; 
	$update_result=mysql_query($update_sql);
			if($parm_debug_on) {
