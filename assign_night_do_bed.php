...
$parm_error_log = '/tmp/assign_night_do_bed.log';
...
$sql="SELECT name,bed,night FROM bed WHERE id>='$start' AND id <= '$end' ";
$result=mysql_query($sql);
while ($rows=mysql_fetch_array($result)) {
	fputs($stdlog, "Select bed :  $rows[0] , $rows[1] ,$rows[2]  \n");
	if (!$rows[2]) {	
	$update_sql = "UPDATE bed set night_nurse='$nurse',night='1'  WHERE   name='$rows[0]' AND bed='$rows[1]' "; 
...
if(isset($_POST[ints])){
 	if (is_array($_POST[ints])) {
			...
			$select_sql="SELECT name,bed,night FROM bed WHERE bed='$value' ";
			fputs($stdlog, "Select start SQL :  $seletc_sql \n");	
			$select_result=mysql_query($select_sql);
			$srow=mysql_fetch_array($select_result);
	if (!$srow[2]) {
	$update_sql = "UPDATE bed set night_nurse='$nurse' ,night='1'  WHERE   bed='$srow[1]'  "; 
	$update_result=mysql_query($update_sql);
			if($parm_debug_on) {
...
header("Location: ./assign_night_bed.php"); 
