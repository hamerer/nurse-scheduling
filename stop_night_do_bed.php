...
$parm_error_log = '/tmp/stop_night_do_bed.log';
...
while ($rows=mysql_fetch_array($result)) {
	...
	if ($rows[2]) {	
	$update_sql = "UPDATE bed set night_nurse='',night='0'  WHERE   name='$rows[0]' AND bed='$rows[1]' "; 
...
if ($nurse!="") {
	$update_sql = "UPDATE bed set night_nurse='',night='0'  WHERE  night_nurse='$nurse'  "; 
	$update_result=mysql_query($update_sql);
	if($parm_debug_on) {
     		fputs($stdlog, "update  sql :  $update_sql \n");
	} 
...
if(isset($_POST[ints])){
 	if (is_array($_POST[ints])) {
		...
		if ($srow[2]) {
			$update_sql = "UPDATE bed set night_nurse='' ,night='0'  WHERE   bed='$srow[1]'  "; 
			$update_result=mysql_query($update_sql);
			if($parm_debug_on) {
...
header("Location: ./stop_night_bed.php"); 
